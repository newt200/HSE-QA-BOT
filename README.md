...
Идея простая: пользователь задает вопрос обычным текстом, а бот пытается сопоставить его с одним из FAQ-вопросов в базе и вернуть связанный с ним готовый ответ.
...
1. Берем эмбеддинг пользовательского запроса с помощью `SentenceTransformer`.
2. Ищем ближайшие кандидаты по FAISS (быстрое приближенное/точное векторное сходство).
